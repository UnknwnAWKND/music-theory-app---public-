...
// Keep the existing key so Block 3 does not erase valid local Phase 1 progress.
...
async function renderHome() {
  ...
  root.innerHTML = shell(`<header class="page-header"><div><div class="eyebrow">Music Theory</div><h1>Home</h1></div></header>
    <section class="focus-card">
      ...
      <p>${esc(focusCopy)}</p>
      <button class="primary" id="continueLearning" type="button">${phase2Passed ? "Review learning" : `Continue Phase ${focusPhase}`}</button>
    </section>
...
async function renderLearn() {
  ...
  const activeSections = [1, 2].map((phase) => {
    ...
    return `<section class="active-phase-section" data-phase="${phase}">
      ...
      <section class="checkpoint-card ${checkpointReady ? "" : "locked"}">
        ...
        <h2>${phaseProgress?.checkpointPassedAt ? "Checkpoint passed" : phase === 1 ? "Representative interval check" : "Representative major-scale check"}</h2>
  ...
  root.innerHTML = shell(`${activeSections}
    <section class="future-phases"><div class="eyebrow">Later blocks</div>${CURRICULUM_PHASES.filter((phase) => phase.phase >= 3).map((phase) => `<div class="future-phase"><span>Phase ${phase.phase}</span><strong>${esc(phase.title)}</strong><small>Not built yet</small></div>`).join("")}</section>`, "learn");
  bindNav();
...
async function renderSettings() {
  root.innerHTML = shell(`<header class="page-header"><div><div class="eyebrow">Profile</div><h1>Settings</h1></div></header>
    <section class="settings-screen">
      ...
      <div class="soft-note">Curriculum version: ${esc(CURRICULUM_VERSION)} · Phases 1–2 active · ${SKILLS.length} lessons</div>
    </section>`, "profile");
  bindNav();
