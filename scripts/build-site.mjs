...
await writeFile(`${OUT}/app-block5.js`, transformBlock5App(block4App));
await writeFile(`${OUT}/index.html`, transformBlock5Index(block4Index));

const sourceConfig = existsSync("web/config.js") ? await readFile("web/config.js", "utf8") : "";
const url = process.env.SUPABASE_URL?.trim();
const key = process.env.SUPABASE_PUBLISHABLE_KEY?.trim();
if (url && key) {
  await writeFile(`${OUT}/config.js`, `globalThis.__THEORY_TUTOR_CONFIG__ = Object.freeze({\n  buildVersion: \"rebuild-block5-phase4-diatonic-chords\",\n  supabaseUrl: ${JSON.stringify(url)},\n  supabasePublishableKey: ${JSON.stringify(key)},\n});\n`);
} else if (!sourceConfig.trim()) {
  await writeFile(`${OUT}/config.js`, `globalThis.__THEORY_TUTOR_CONFIG__ = Object.freeze({ buildVersion: \"rebuild-block5-phase4-diatonic-chords\", supabaseUrl: \"\", supabasePublishableKey: \"\" });\n`);
}

console.log("Built Block 5 Phase 4 diatonic-harmony site in ./site");
