...
const INVERSION_MAP: Readonly<Record<Phase1IntervalName, Phase1IntervalName>> = Object.freeze({
  P1: "P8",
  P8: "P1",
  P5: "P4",
  P4: "P5",
...
export function intervalAbove(root: Note, interval: IntervalSpec): Note {
...
export function invertSimpleIntervalNumber(number: number): number {
  if (!Number.isInteger(number) || number < 1 || number > 8) throw new Error(`Simple interval number must be 1–8, received ${number}`);
  return 9 - number;
}

export function invertPhase1Interval(name: Phase1IntervalName): Phase1IntervalName {
  return INVERSION_MAP[name];
}

export function inversionQuality(quality: IntervalQuality): IntervalQuality {
  if (quality === "perfect") return "perfect";
  if (quality === "major") return "minor";
  if (quality === "minor") return "major";
  if (quality === "augmented") return "diminished";
  return "augmented";
}

export function isSimpleIntervalNumber(number: number): boolean {
  return Number.isInteger(number) && number >= 1 && number <= 8;
}

export function simpleToCompoundIntervalNumber(number: number): number {
  if (!isSimpleIntervalNumber(number)) throw new Error(`Expected a simple interval number 1–8, received ${number}`);
  return number + 7;
}
