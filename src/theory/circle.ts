...
function relativeMinorForMajor(majorName: string): string {
  return formatNote(relativeMinorTonic(parseNote(majorName)));
}

export const CIRCLE_MAJOR_ORDER = ["C", "G", "D", "A", "E", "B", "F#", "Db", "Ab", "Eb", "Bb", "F"] as const;

export const CIRCLE_POSITIONS: readonly CirclePosition[] = Object.freeze(
  RAW_MAJOR_ALIASES.map((aliases, index) => Object.freeze({
    index,
...
function spellingFamily(name: string): "sharp" | "flat" | "natural" {
  const text = normalized(name);
  if (text.includes("♯")) return "sharp";
  if (text.includes("♭")) return "flat";
  return "natural";
}

export function circlePositionForMajorKey(keyName: string): CirclePosition {
  const key = normalized(keyName);
  const position = CIRCLE_POSITIONS.find((item) => item.majorAliases.includes(key));
  if (!position) throw new Error(`Unsupported Circle-of-Fifths major-key spelling: ${keyName}`);
  return position;
}

export function circleMajorDisplayLabel(positionIndex: number): string {
  return CIRCLE_POSITIONS[mod(positionIndex, 12)].majorAliases.join(" / ");
}

export function circleMinorDisplayLabel(positionIndex: number): string {
  return CIRCLE_POSITIONS[mod(positionIndex, 12)].relativeMinorAliases.map((name) => `${name}m`).join(" / ");
}

export function circleDistanceBetweenMajors(a: string, b: string): number {
...
function aliasForTarget(target: CirclePosition, sourceKey: string, direction: CircleDirection): string {
  if (target.majorAliases.length === 1) return target.majorAliases[0];
  const family = spellingFamily(sourceKey);
  if (family === "flat") return target.majorAliases.find((name) => name.includes("♭")) ?? target.majorAliases.at(-1)!;
  if (family === "sharp") return target.majorAliases.find((name) => name.includes("♯")) ?? target.majorAliases[0];
  if (direction === "counterclockwise") return target.majorAliases.find((name) => name.includes("♭")) ?? target.majorAliases.at(-1)!;
  return target.majorAliases.find((name) => name.includes("♯")) ?? target.majorAliases[0];
}

export function circleMoveMajor(keyName: string, direction: CircleDirection): CircleMove {
  ...
  if (actual !== expected) throw new Error(`Circle movement mismatch: ${keyName} -> ${to}`);
...
function majorScalePitchClasses(keyName: string): Set<number> {
  return new Set(majorScaleNames(keyName).map((note) => pitchClass(parseNote(note))));
}

export function sharedMajorScalePitchClasses(a: string, b: string): number[] {
  const left = majorScalePitchClasses(a);
  const right = majorScalePitchClasses(b);
  return [...left].filter((pc) => right.has(pc)).sort((x, y) => x - y);
}

export function sharedMajorScaleNoteCount(a: string, b: string): number {
  return sharedMajorScalePitchClasses(a, b).length;
}

export function areAdjacentMajorKeys(a: string, b: string): boolean {
  return circleDistanceBetweenMajors(a, b) === 1;
}

export function relativeMinorAtMajorKey(majorName: string): string {
  return relativeMinorForMajor(majorName);
}

export function closelyRelatedKeysForMajor(homeMajor: string): { majors: string[]; minors: string[] } {
...
export function isFarSideMajorTarget(homeMajor: string, targetMajor: string): boolean {
  return circleDistanceBetweenMajors(homeMajor, targetMajor) >= 4;
}

export function farSideMajorTargets(homeMajor: string): string[] {
  ...
  return CIRCLE_POSITIONS
    ...
    .map((position) => {
      if (position.majorAliases.length === 1) return position.majorAliases[0];
      if (family === "flat") return position.majorAliases.find((name) => name.includes("♭")) ?? position.majorAliases.at(-1)!;
      if (family === "sharp") return position.majorAliases.find((name) => name.includes("♯")) ?? position.majorAliases[0];
      if (position.index === 7) return position.majorAliases.find((name) => name.includes("♭")) ?? position.majorAliases[0];
      return position.majorAliases[0];
    });
}

export function selectFarSideMajorTarget(homeMajor: string, index = 0): string {
  const candidates = farSideMajorTargets(homeMajor);
  if (!candidates.length) throw new Error(`No far-side target available for ${homeMajor}`);
  return candidates[mod(index, candidates.length)];
}

export function parseMajorRomanProgression(input: string | readonly string[]): string[] {
  ...
  if (!normalizedTokens.length) throw new Error("Enter at least one Roman numeral.");
  for (const token of normalizedTokens) majorDegreeForRoman(token);
  return normalizedTokens;
}

export function transposeMajorRomanProgression(targetMajor: string, progression: string | readonly string[]): TransposedRomanChord[] {
...
export const PHASE6_FALLBACK_ROMAN_PROGRESSION = ["I", "V", "vi", "IV"] as const;

export function resolveFarSideProgression(saved?: SavedProgressionLike | null): { source: "saved" | "fallback"; romanNumerals: string[] } {
