...
function acquisitionEvidence(attempts: readonly LearningAttempt[]) {
  const successes = attempts.filter(
    (x) => x.context === "acquisition" && x.directEvidence && x.independent && x.outcome === "correct",
  );
  return {
...
function reviewSessionColdResults(attempts: readonly LearningAttempt[]): Array<{ sessionId: string; result: "success" | "failure"; time: number }> {
  const firstBySession = new Map<string, LearningAttempt>();
  for (const attempt of attempts) {
    if (attempt.context !== "review" || !attempt.coldProbe || !attempt.directEvidence || !attempt.independent) continue;
...
export function deriveSkillEvidence(
  ...
  const evidenceBasis: DerivedSkillEvidence["evidenceBasis"] = sources.size === 0 ? "none" : sources.size > 1 ? "mixed" : sources.has("self-report") ? "self-report" : "objective";
  ...
  const ready =
    acquisition.successes >= policy.readyIndependentSuccesses &&
...
export function nextAcquisitionAction(
  ...
  const attempts = chronological(attemptsInput);
  if (attempts.length === 0) return "continue-independent";
  const latest = attempts[attempts.length - 1];
  if (latest.outcome === "correct") return "continue-independent";
  if (latest.outcome === "hinted" || latest.outcome === "revealed") return "scaffold-and-retry";
  ...
  for (let i = sessionAttempts.length - 1; i >= 0; i--) {
    ...
    if (!x.independent || !x.directEvidence) continue;
