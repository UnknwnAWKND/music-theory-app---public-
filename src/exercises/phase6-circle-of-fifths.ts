...
const HOME_KEYS = ["C", "G", "F", "D", "Bb", "A", "Eb", "E", "Ab", "B", "F#", "Gb", "Db", "C#"] as const;
const PROGRESSIONS = [
  ["I", "V", "vi", "IV"],
  ["ii", "V", "I"],
  ["I", "IV", "V", "I"],
...
function homeAt(index: number): string { return HOME_KEYS[mod(index * 5 + Math.floor(index / HOME_KEYS.length), HOME_KEYS.length)]; }

function metadata(homeKey: string, targetKey: string | undefined, competencies: readonly string[], extra: Record<string, unknown> = {}) {
  return {
...
function neighborAnswer(home: string, direction: CircleDirection) {
...
function lesson1Generator(index: number): Exercise {
  ...
  return withReview(index, (safe) => {
    ...
    if (variant === 0 || variant === 1) {
      const direction: CircleDirection = variant === 0 ? "clockwise" : "counterclockwise";
...
export const PHASE6_CIRCLE_GENERATORS: ReadonlyMap<string, ExerciseGenerator> = new Map([
...
export function phase6ExerciseForSkill(skillId: string, index = 0): Exercise | undefined {
