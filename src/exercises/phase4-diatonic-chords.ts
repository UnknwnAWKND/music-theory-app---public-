import {
  basicChordFunction,
  deriveDiatonicChord,
  deriveDiatonicHarmony,
  formatNote,
  functionExplanation,
  parseNote,
  pitchClass,
  transposeRomanProgression,
  type HarmonyScaleForm,
  type TriadQuality,
...
const ROOTS = ["C", "F#", "Eb", "A", "Bb", "E", "G", "C#", "F", "D", "Ab", "B"] as const;
const MINOR_ROOTS = ["A", "F#", "C", "Eb", "G#", "D", "Bb", "E", "C#", "F", "B", "G"] as const;
const QUALITY_CHOICES = ["major", "minor", "diminished", "augmented"] as const;
const SEVENTH_CHOICES = ["major7", "minor7", "dominant7", "half-diminished7", "diminished7", "minor-major7", "augmented-major7"] as const;
const FUNCTION_CHOICES = ["tonic", "predominant", "dominant", "context-dependent"] as const;

function mod(n: number, m: number): number { return ((n % m) + m) % m; }
function rootFor(form: HarmonyScaleForm, index: number): string { return form === "major" ? ROOTS[mod(index, ROOTS.length)] : MINOR_ROOTS[mod(index, MINOR_ROOTS.length)]; }
function formName(form: HarmonyScaleForm): string {
  if (form === "major") return "major";
  if (form === "natural-minor") return "natural minor";
  if (form === "harmonic-minor") return "harmonic minor";
  return "ascending melodic minor";
}
function seventhName(value: string): string {
  return ({ major7: "major7", minor7: "minor7", dominant7: "dominant7", halfDiminished7: "half-diminished7", diminished7: "diminished7", minorMajor7: "minor-major7", augmentedMajor7: "augmented-major7" } as Record<string, string>)[value] ?? value;
}
function qualityLabel(value: TriadQuality): string { return value; }
function pianoName(note: string): string {
  const names = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
  return names[pitchClass(parseNote(note))];
}
function metadata(chord: ReturnType<typeof deriveDiatonicChord>, competencies: readonly string[], extra: Record<string, unknown> = {}) {
  return {
...
function priorFoundationReview(index: number): Exercise {
  const item = REVIEW_SOURCES[mod(Math.floor(index / 13), REVIEW_SOURCES.length)]();
  if (!item) throw new Error("Missing prior-foundation review generator");
...
function withReview(index: number, own: (index: number) => Exercise): Exercise { return index > 0 && index % 13 === 0 ? priorFoundationReview(index) : own(index); }

function constructTriad(skillId: string, form: HarmonyScaleForm, index: number, competencies: readonly string[]): Exercise {
...
function identifyTriadQuality(skillId: string, form: HarmonyScaleForm, index: number, competencies: readonly string[]): Exercise {
...
function identifyRoman(skillId: string, form: HarmonyScaleForm, index: number, competencies: readonly string[]): Exercise {
...
function stackingGenerator(index: number): Exercise {
  const skillId = PHASE4_DIATONIC_CHORD_SKILL_IDS[0];
  return withReview(index, (safe) => {
    if (safe % 3 === 0) return constructTriad(skillId, "major", safe, ["stacking-thirds"]);
...
function triadFormGenerator(skillId: string, form: HarmonyScaleForm, index: number, competencies: readonly string[]): Exercise {
  return withReview(index, (safe) => {
    if (safe % 3 === 0) return constructTriad(skillId, form, safe, competencies);
    if (safe % 3 === 1) return identifyTriadQuality(skillId, form, safe, competencies);
    return identifyRoman(skillId, form, safe, [...competencies, "roman-numeral-translation"]);
  });
}

function harmonicGenerator(index: number): Exercise {
  const skillId = PHASE4_DIATONIC_CHORD_SKILL_IDS[3];
  return withReview(index, (safe) => {
    if (safe % 5 < 3) return triadFormGenerator(skillId, "harmonic-minor", safe, ["harmonic-minor-triads"]);
    if (safe % 5 === 3) {
...
function seventhGenerator(index: number): Exercise {
  const skillId = PHASE4_DIATONIC_CHORD_SKILL_IDS[5];
  return withReview(index, (safe) => {
    const forms: HarmonyScaleForm[] = ["major", "natural-minor", "harmonic-minor", "melodic-minor-ascending"];
...
function functionGenerator(index: number): Exercise {
...
const MAJOR_PROGRESSIONS = [[1,5,6,4],[6,4,1,5],[2,5,1],[1,6,2,5]] as const;
const MINOR_PROGRESSION = [1,6,3,7] as const;
function progressionGenerator(index: number): Exercise {
  const skillId = PHASE4_DIATONIC_CHORD_SKILL_IDS[7];
  return withReview(index, (safe) => {
    const minor = safe % 5 === 4;
    const form: HarmonyScaleForm = minor ? "natural-minor" : "major";
...
function ownProgressionGenerator(index: number): Exercise {
  const skillId = PHASE4_DIATONIC_CHORD_SKILL_IDS[8];
  return withReview(index, (safe) => {
    const form: HarmonyScaleForm = safe % 3 === 0 ? "natural-minor" : "major";
    ...
    const wrongQuality: TriadQuality = chord.triadQuality === "major" ? "minor" : "major";
...
export const PHASE4_DIATONIC_CHORD_GENERATORS: ReadonlyMap<string, ExerciseGenerator> = new Map([
...
export function phase4ExerciseForSkill(skillId: string, index = 0): Exercise | undefined { return PHASE4_DIATONIC_CHORD_GENERATORS.get(skillId)?.(index); }
