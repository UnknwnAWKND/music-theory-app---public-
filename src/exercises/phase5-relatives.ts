...
function pairAt(index: number): RelativeKeyPair { return RELATIVE_KEY_PAIRS[mod(index, RELATIVE_KEY_PAIRS.length)]; }
function isEasy(pair: RelativeKeyPair): boolean { return ["C", "G", "F", "D"].includes(pair.major); }

function metadata(pair: RelativeKeyPair, competencies: readonly string[], extra: Record<string, unknown> = {}) {
  return {
...
function priorReview(index: number): Exercise {
  ...
  if (!item) throw new Error("Missing Phase 5 prior-foundation review item");
  return {
    ...item,
    metadata: {
      ...(item.metadata ?? {}),
...
function withReview(index: number, own: (safe: number) => Exercise): Exercise {
...
function lesson1Generator(index: number): Exercise {
  const skillId = PHASE5_RELATIVE_SKILL_IDS[0];
  return withReview(index, (safe) => {
    if (safe % 5 < 3) return sharedCollectionQuestion(skillId, safe);
...
function renumberQuestion(skillId: string, index: number): Exercise {
...
function lesson2Generator(index: number): Exercise {
  const skillId = PHASE5_RELATIVE_SKILL_IDS[1];
  return withReview(index, (safe) => {
    if (safe % 5 < 4) return renumberQuestion(skillId, safe);
...
function directRelativeQuestion(skillId: string, index: number, automatic = false): Exercise {
...
function lesson3Generator(index: number): Exercise {
  const skillId = PHASE5_RELATIVE_SKILL_IDS[2];
  return withReview(index, (safe) => {
    if (safe % 4 < 3) return directRelativeQuestion(skillId, safe + Math.floor(safe / 4), true);
...
function lesson4Generator(index: number): Exercise {
  const skillId = PHASE5_RELATIVE_SKILL_IDS[3];
  return withReview(index, (safe) => {
    const pairIndex = mod(safe * 7 + Math.floor(safe / 15), RELATIVE_KEY_PAIRS.length);
    if (safe % 6 < 4) return directRelativeQuestion(skillId, pairIndex * 2 + (safe % 2), true);
    if (safe % 6 === 4) return renumberQuestion(skillId, safe + 31);
    return sharedCollectionQuestion(skillId, safe + 37);
  });
}

export const PHASE5_RELATIVE_GENERATORS: ReadonlyMap<string, ExerciseGenerator> = new Map([
...
export function phase5ExerciseForSkill(skillId: string, index = 0): Exercise | undefined {
