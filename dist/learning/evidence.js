...
export function evidencePolicyForModes(modes = []) {
    ...
    const readinessMode = onlyApplication ? "application" : hasRelationalEvidence ? "varied" : "atomic";
...
function firstSubmission(attempt) {
    if (typeof attempt.firstSubmission === "boolean")
        return attempt.firstSubmission;
    if (typeof attempt.submissionIndex === "number")
        return attempt.submissionIndex === 1;
...
function isResponse(attempt) {
    return eventKind(attempt) === "response";
}
function isCleanFirstResponse(attempt) {
...
function legacyReady(attempts, policy) {
    if (!policy.preserveLegacyReady)
        return { ready: false };
    const acquisition = attempts.filter((x) => isLegacy(x)
        ...
        && x.independent
        && x.outcome === "correct");
    if (acquisition.length < 3 || new Set(acquisition.map((x) => x.promptSignature)).size < 3)
        return { ready: false };
    const lastLegacyAcquisition = [...attempts].reverse().find((x) => isLegacy(x)
        ...
        && x.independent
        && (x.outcome === "correct" || x.outcome === "incorrect"));
    if (lastLegacyAcquisition?.outcome !== "correct")
        return { ready: false };
    const successes = [];
    ...
    for (const attempt of acquisition) {
        ...
        if (successes.length >= 3 && signatures.size >= 3)
            return { ready: true, establishedAt: attempt.occurredAt };
    }
    return { ready: false };
}
function currentReadiness(attempts, mode) {
    ...
    for (let i = 0; i < cleanCorrect.length; i++) {
        ...
        if (basis !== "none")
            return { ready: true, basis, establishedAt: cleanCorrect[i].occurredAt };
    }
...
export function normalizeLearningAttempt(input, previousInput = []) {
    ...
    const priorColdFailure = previous.some((x) => x.sessionId === input.sessionId
        ...
        && isCleanFirstResponse(x)
        && x.outcome === "incorrect");
    const stage = input.stage ?? (kind === "response" && priorColdFailure && !input.coldProbe && input.context === "review"
        ? "relearning"
        : kind === "response" && (submissionIndex ?? 1) > 1
            ? "retry"
            : "initial");
    const priorRelevantExposureAt = input.priorRelevantExposureAt ?? prior?.occurredAt;
    const elapsedSinceRelevantExposureMs = input.elapsedSinceRelevantExposureMs ?? (priorRelevantExposureAt
...
export function deriveSkillEvidence(attemptsInput, policy = DEFAULT_EVIDENCE_POLICY) {
    ...
    const evidenceBasis = directSources.size === 0
        ? "none"
    ...
    const readinessBasis = current.ready ? current.basis : legacy.ready ? "legacy-v1" : "none";
    ...
    const confusions = {};
    for (const attempt of attempts) {
        if (!attempt.confusionWith)
            continue;
        confusions[attempt.confusionWith] = (confusions[attempt.confusionWith] ?? 0) + 1;
    }
    const countMode = (mode) => currentFirstCorrect.filter((x) => x.responseMode === mode).length;
    return {
        state,
        ready,
        retained,
...
export function nextAcquisitionAction(attemptsInput, policy = DEFAULT_EVIDENCE_POLICY) {
    ...
    if (latest.outcome === "correct")
        return "continue-independent";
    if (guidance(latest) !== "none" || latest.outcome === "hinted" || latest.outcome === "revealed")
    ...
    for (let i = sessionAttempts.length - 1; i >= 0; i--) {
        ...
        if (!x.independent || !x.directEvidence || guidance(x) !== "none")
            continue;
