function inferredOptions(phase, tags = [], optional = false) {
    if (tags.includes("number"))
        return { priority: "foundation", recurrenceWeight: 5, acquisitionRoundSize: 30, thread: "interval" };
    if (tags.includes("interval"))
        return { priority: "foundation", recurrenceWeight: 5, acquisitionRoundSize: 30, thread: "interval" };
    if (tags.includes("degree"))
        return { priority: "foundation", recurrenceWeight: 4, acquisitionRoundSize: 30, thread: "number-system" };
    if (tags.includes("scale") && phase <= 3)
        return { priority: "foundation", recurrenceWeight: 4, acquisitionRoundSize: 30, thread: "scale" };
    if (tags.includes("chord") && phase <= 4)
        return { priority: "foundation", recurrenceWeight: 4, acquisitionRoundSize: 30, thread: "triad" };
    if (tags.includes("function"))
        return { priority: "core", recurrenceWeight: 3, acquisitionRoundSize: 30, thread: "function" };
    if (tags.includes("melody"))
        return { priority: "foundation", recurrenceWeight: 4, acquisitionRoundSize: 30, thread: "chord-tone" };
    if (tags.includes("guitar"))
        return { priority: "core", recurrenceWeight: 2, acquisitionRoundSize: 30, thread: "guitar" };
    if (phase === 11 || optional)
        return { priority: "extension", recurrenceWeight: 1, acquisitionRoundSize: 30 };
    return { priority: "core", recurrenceWeight: 2, acquisitionRoundSize: 30 };
}
const s = (id, phase, title, prerequisites, evidence = ["construct", "identify"], tags = [], optional = false, options = {}) => ({
    id,
    phase,
    title,
    prerequisites,
