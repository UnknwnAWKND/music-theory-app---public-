...
const MODE_PARALLEL_ALTERATIONS = { lydian: "major with ♯4", mixolydian: "major with ♭7", dorian: "natural minor with raised 6", phrygian: "natural minor with ♭2", locrian: "natural minor with ♭2 and ♭5" };
const TRIAD_RULE = {
...
function intervalLesson(skillId) {
...
export function lessonForSkill(skillId) {
    ...
    if (TRIAD_RULE[skillId])
        return { skillId, title: skill.title, summary: TRIAD_RULE[skillId], rule: TRIAD_RULE[skillId] };
    if (SEVENTH_RULE[skillId])
        return { skillId, title: skill.title, summary: SEVENTH_RULE[skillId], rule: SEVENTH_RULE[skillId] };
    if (skillId === "mode.major-family")
        return { skillId, title: skill.title, summary: `Parallel comparison: Lydian = ${MODE_PARALLEL_ALTERATIONS.lydian}; Mixolydian = ${MODE_PARALLEL_ALTERATIONS.mixolydian}. Keep the same tonic.` };
    if (skillId === "mode.minor-family")
        return { skillId, title: skill.title, summary: `Parallel comparison: Dorian = ${MODE_PARALLEL_ALTERATIONS.dorian}; Phrygian = ${MODE_PARALLEL_ALTERATIONS.phrygian}. Keep the same tonic.` };
    if (skillId === "mode.locrian")
        return { skillId, title: skill.title, summary: `Locrian = ${MODE_PARALLEL_ALTERATIONS.locrian}. Its tonic triad is diminished, which makes stable tonic treatment less common in tonal styles.` };
    const found = exact[skillId];
    if (!found)
        throw new Error(`No lesson content for skill ${skillId}`);
