import { MINOR_PITCH_CLASS_ROOTS, NATURAL_MINOR_DEGREE_INTERVALS, NATURAL_MINOR_STEP_PATTERN, classicalMelodicMinorNames, conventionalMinorRootsForPitchClass, harmonicMinorAugmentedSecond, harmonicMinorScaleNames, melodicMinorAscendingScaleNames, minorLeadingToneName, minorScaleDegreeNote, naturalMinorScaleNames, parseNote, pitchClass, } from "../theory/index.js";
...
const PIANO_PITCH_NAMES = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
function mod(n, m) {
    return ((n % m) + m) % m;
}
function pianoPitchName(noteName) {
    return PIANO_PITCH_NAMES[pitchClass(parseNote(noteName))];
}
function rootForPitchClassIndex(pcIndex, spellingRound = 0) {
...
function rootForBuildIndex(index) {
    const group = Math.floor(Math.max(0, index) / 4);
    return rootForPitchClassIndex(group, Math.floor(group / 12));
}
function rootForRecallIndex(index) {
    const safe = Math.max(0, index);
    return rootForPitchClassIndex(safe, Math.floor(safe / 12));
}
function scaleForForm(root, form) {
    if (form === "natural")
        return naturalMinorScaleNames(root);
    if (form === "harmonic")
        return harmonicMinorScaleNames(root);
    if (form === "melodic-ascending")
        return melodicMinorAscendingScaleNames(root);
...
function scaleMetadata(root, form, direction, responseMode, checkpointCompetencies, extra = {}) {
...
function crossPhaseFoundationReview(index) {
    const safe = Math.max(0, index);
    if (safe % 2 === 0) {
        ...
        if (!item)
            throw new Error(`Missing Phase 1 review generator for ${skillId}`);
        return {
            ...item,
...
function naturalFormulaQuestion(skillId, index) {
...
function naturalFormulaStep(skillId, index) {
...
function naturalIntervalConnection(skillId, index) {
...
function naturalAppliedScale(skillId, index) {
...
function buildNaturalScale(skillId, root, index, recall = false) {
    const scale = naturalMinorScaleNames(root);
    return createExercise({
        skillId,
...
function missingMinorNote(skillId, root, form, index, recall = false) {
    const scale = scaleForForm(root, form);
...
function requestedMinorDegree(skillId, root, form, index, recall = false) {
...
function identifyNaturalDegree(skillId, root, index) {
...
function buildHarmonicScale(skillId, root, index, recall = false) {
...
function harmonicRaisedDegree(skillId, index) {
...
function harmonicLeadingTone(skillId, root, index) {
...
function harmonicAugmentedSecond(skillId, root, index) {
...
function harmonicReason(skillId, index) {
    return createExercise({
        skillId,
...
function buildMelodicAscending(skillId, root, index, recall = false) {
...
function buildMelodicDescending(skillId, root, index, recall = false) {
...
function melodicChangedDegrees(skillId, index) {
    return createExercise({
        skillId,
...
function melodicWhySix(skillId, index) {
    return createExercise({
        skillId,
...
function descendingConvention(skillId, index) {
    return createExercise({
        skillId,
...
function formDiscrimination(skillId, root, form, index) {
    const scale = scaleForForm(root, form);
...
function phase3NaturalFormulaGenerator(index) {
    const safe = Math.max(0, index);
    if (safe % 13 === 12)
        return crossPhaseFoundationReview(safe);
    switch (safe % 4) {
...
function phase3NaturalRootsGenerator(index) {
    const safe = Math.max(0, index);
    if (safe % 17 === 16)
        return crossPhaseFoundationReview(safe + 31);
...
function phase3HarmonicGenerator(index) {
    const safe = Math.max(0, index);
    if (safe % 17 === 16)
        return crossPhaseFoundationReview(safe + 53);
...
function phase3MelodicGenerator(index) {
    const safe = Math.max(0, index);
    if (safe % 17 === 16)
        return crossPhaseFoundationReview(safe + 79);
    const root = rootForRecallIndex(Math.floor(safe / 2));
    switch (safe % 7) {
...
function phase3RecallGenerator(index) {
    const safe = Math.max(0, index);
    if (safe % 19 === 18)
        return crossPhaseFoundationReview(safe + 101);
    const root = rootForRecallIndex(safe);
    const cycle = mod(Math.floor(safe / 12), 6);
    if (cycle === 0)
        return buildNaturalScale(PHASE3_MINOR_SCALE_SKILL_IDS[4], root, safe, true);
    if (cycle === 1)
        return buildHarmonicScale(PHASE3_MINOR_SCALE_SKILL_IDS[4], root, safe, true);
    if (cycle === 2)
        return buildMelodicAscending(PHASE3_MINOR_SCALE_SKILL_IDS[4], root, safe, true);
    if (cycle === 3)
        return buildMelodicDescending(PHASE3_MINOR_SCALE_SKILL_IDS[4], root, safe, true);
    if (cycle === 4) {
        const form = ["natural", "harmonic", "melodic-ascending"][mod(safe, 3)];
        return requestedMinorDegree(PHASE3_MINOR_SCALE_SKILL_IDS[4], root, form, safe, true);
    }
    const form = ["natural", "harmonic", "melodic-ascending", "melodic-descending"][mod(safe, 4)];
    return formDiscrimination(PHASE3_MINOR_SCALE_SKILL_IDS[4], root, form, safe);
}
export const PHASE3_MINOR_SCALE_GENERATORS = new Map([
...
export function phase3ExerciseForSkill(skillId, index = 0) {
    return PHASE3_MINOR_SCALE_GENERATORS.get(skillId)?.(index);
}
export function phase3BalancedRootNames(count = 60) {
