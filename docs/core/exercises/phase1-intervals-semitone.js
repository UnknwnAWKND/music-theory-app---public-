import { INTERVALS, PHASE1_INTERVAL_NAMES, } from "../theory/index.js";
import { createExercise } from "./generators.js";
import { PHASE1_INTERVAL_GENERATORS as BASE_PHASE1_INTERVAL_GENERATORS, } from "./phase1-intervals.js";
 ...
 */
const SEMITONE_POOLS = Object.freeze({
    [L1]: ["P1", "P8"],
    [L2]: ["P5", "P1", "P8"],
    [L3]: ["P4", "P5", "P1", "P8"],
...
const QUALITY_DISCRIMINATION_PAIRS = Object.freeze({
    [L4]: [["m3", "M3"]],
    [L5]: [["m6", "M6"], ["m3", "M3"]],
...
function readableInterval(interval) {
    if (interval === "P1")
...
function semitoneWord(count) {
    return `${count} semitone${count === 1 ? "" : "s"}`;
}
function articleFor(text) {
    return /^[aeiou]/i.test(text) ? "an" : "a";
}
function makeIntervalToSemitones(skillId, interval, index) {
    const spec = INTERVALS[interval];
...
function makeSemitonesToInterval(skillId, interval, index) {
    const spec = INTERVALS[interval];
    const name = readableInterval(interval);
...
function makeQualityDiscrimination(skillId, pair, index) {
...
function semitoneIntervalFor(skillId, index) {
    const pool = SEMITONE_POOLS[skillId];
    if (!pool?.length)
        throw new Error(`No semitone pool for ${skillId}`);
    const cycle = Math.floor(Math.max(0, index) / 10);
    return pool[cycle % pool.length];
}
function discriminationPairFor(skillId, index) {
...
function wrapGenerator(skillId, base) {
    return (index) => {
        ...
        if (skillId === L1) {
            if (slot === 0)
                return makeIntervalToSemitones(skillId, "P1", safe);
            if (slot === 1)
                return makeSemitonesToInterval(skillId, "P1", safe);
            if (slot === 2)
                return makeIntervalToSemitones(skillId, "P8", safe);
            if (slot === 3)
                return makeSemitonesToInterval(skillId, "P8", safe);
            return base(safe);
        }
        ...
        if (slot === 0)
            return makeIntervalToSemitones(skillId, semitoneInterval, safe);
        if (slot === 1)
            return makeSemitonesToInterval(skillId, semitoneInterval, safe);
        if (slot === 2) {
            const pair = discriminationPairFor(skillId, safe);
            if (pair)
                return makeQualityDiscrimination(skillId, pair, safe);
        }
        return base(safe);
    };
}
export const PHASE1_INTERVAL_GENERATORS = new Map([...BASE_PHASE1_INTERVAL_GENERATORS].map(([skillId, generator]) => [skillId, wrapGenerator(skillId, generator)]));
export function phase1ExerciseForSkill(skillId, index = 0) {
    return PHASE1_INTERVAL_GENERATORS.get(skillId)?.(index);
}
export function phase1SemitoneIntervalsForSkill(skillId) {
