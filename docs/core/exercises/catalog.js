...
function majorExercise(skillId, index) {
    ...
    if (skillId === "major.membership") {
        ...
        const scale = majorScale(tonicNote);
        const inScale = index % 2 === 0;
        const chromaticIntervals = [INTERVALS.m2, INTERVALS.A4, INTERVALS.m7];
...
function qaVariedConceptExercise(skillId, index) {
    ...
    if (skillId === "function.predominant")
        return odd
    ...
    if (skillId === "function.basic-flow")
        return odd
    ...
    if (skillId === "guitar.alternate-tunings")
        return selfCheck(skillId, index, odd
