import { MAJOR_PITCH_CLASS_ROOTS, MAJOR_SCALE_DEGREE_INTERVALS, MAJOR_SCALE_STEP_PATTERN, conventionalMajorRootsForPitchClass, majorScaleDegreeNote, majorScaleNames, parseNote, pitchClass, scaleDegreeName, } from "../theory/index.js";
...
const SIMPLE_APPLICATION_ROOTS = ["C", "G", "F", "D"];
...
function scaleMetadata(root, direction, responseMode, checkpointCompetencies, extra = {}) {
...
function crossPhaseIntervalReview(index) {
    const safe = Math.max(0, index);
    ...
    if (!item)
        throw new Error(`Missing Phase 1 interval review generator for ${skillId}`);
    return {
        ...item,
        metadata: {
            ...(item.metadata ?? {}),
...
function formulaSequence(skillId, index) {
...
function formulaStepLocation(skillId, index) {
...
function formulaIntervalConnection(skillId, index) {
...
function formulaAppliedScale(skillId, index) {
...
function degreeNameQuestion(skillId, index) {
...
function degreeNumberQuestion(skillId, index) {
...
function degreeNoteQuestion(skillId, index) {
    const root = SIMPLE_APPLICATION_ROOTS[mod(Math.floor(index / 4), SIMPLE_APPLICATION_ROOTS.length)];
    const degree = mod(index * 5, 7) + 1;
...
function noteDegreeQuestion(skillId, index) {
...
function buildFullScale(skillId, root, index, recall = false) {
    const scale = majorScaleNames(root);
...
function missingScaleNote(skillId, root, index, recall = false) {
    const scale = majorScaleNames(root);
...
function requestedDegreeNote(skillId, root, index, recall = false) {
...
function identifyDegreeInKey(skillId, root, index, recall = false) {
...
function phase2FormulaGenerator(index) {
    const safe = Math.max(0, index);
    if (safe % 9 === 8)
        return crossPhaseIntervalReview(safe);
    switch (safe % 4) {
...
function phase2DegreeGenerator(index) {
    const safe = Math.max(0, index);
    if (safe % 12 === 11)
        return crossPhaseIntervalReview(safe + 19);
    switch (safe % 4) {
...
function phase2BuildGenerator(index) {
    const safe = Math.max(0, index);
    if (safe % 12 === 11)
        return crossPhaseIntervalReview(safe + 37);
...
function phase2RecallGenerator(index) {
    const safe = Math.max(0, index);
    if (safe % 10 === 9)
        return crossPhaseIntervalReview(safe + 71);
    const root = rootForRecallIndex(safe);
    const batch = mod(Math.floor(safe / 12), 5);
    if (batch === 0 || batch === 3)
        return buildFullScale(PHASE2_MAJOR_SCALE_SKILL_IDS[3], root, safe, true);
    if (batch === 1)
        return requestedDegreeNote(PHASE2_MAJOR_SCALE_SKILL_IDS[3], root, safe, true);
    if (batch === 2)
        return missingScaleNote(PHASE2_MAJOR_SCALE_SKILL_IDS[3], root, safe, true);
    return identifyDegreeInKey(PHASE2_MAJOR_SCALE_SKILL_IDS[3], root, safe, true);
}
export const PHASE2_MAJOR_SCALE_GENERATORS = new Map([
...
export function phase2ExerciseForSkill(skillId, index = 0) {
    return PHASE2_MAJOR_SCALE_GENERATORS.get(skillId)?.(index);
}
export function phase2BalancedRootNames(count = 60) {
