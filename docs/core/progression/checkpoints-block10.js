import { allCheckpointDefinitions as priorAllCheckpointDefinitions, checkpointDefinition as priorCheckpointDefinition, evaluateCheckpoint as priorEvaluateCheckpoint, phaseCoreReady, placementDefinition, placementPrerequisitePhases, recommendStartingPhase, } from "./checkpoints-block8.js";
export { phaseCoreReady, placementDefinition, placementPrerequisitePhases, recommendStartingPhase, };
const CHECKPOINT_ITEMS = 200;
function fullCheckpoint(definition) {
    if (!definition)
        return undefined;
...
export function evaluateCheckpoint(definition, results) {
    ...
    if (results.length >= CHECKPOINT_ITEMS)
        return evaluation;
    return {
        ...evaluation,
        passed: false,
...
export function nextCheckpointCompetency(definition, results) {
    if (!definition.competencies.length || results.length >= CHECKPOINT_ITEMS)
        return undefined;
