...
const daysBetween = (later, earlier) => earlier
    ? Math.max(0, (later.getTime() - new Date(earlier).getTime()) / DAY_MS)
    : 0;
const grade = (rating) => rating === "again" ? 1 : 3;
/**
 ...
 */
export class Fsrs6LongTermSchedulerAdapter {
    policy;
    weights;
    constructor(policy = {}, weights = FSRS6_DEFAULT_WEIGHTS) {
        this.policy = { ...DEFAULT_SCHEDULER_POLICY, ...policy, schedulerVersion: "fsrs-6" };
        if (weights.length !== 21)
            throw new Error("FSRS-6 requires 21 parameters");
    ...
    shortTermStability(stability, g) {
    ...
    recallStability(stability, difficulty, retrievability) {
        return Math.max(0.001, stability * (1
    ...
    wrap(before, after, rating, reviewedAt) {
        return {
            card: after,
            log: {
    ...
    schedule(card, rating, reviewedAt) {
        ...
        if (card.reps === 0 || card.state === "new") {
            ...
            if (elapsed < 1) {
                ...
                if (rating === "good")
                    stability = this.recallStability(card.stability, difficulty, r);
                else {
                    stability = this.forgetStability(card.stability, difficulty, r);
                    lapses += 1;
                }
            }
        }
