...
import { mod, pitchClass } from "./note.js";
...
export function buildSeventh(root, quality) {
    const formulas = {
...
export function identifyTriad(notes) {
    if (notes.length !== 3)
        throw new Error("Triad must contain three notes");
    ...
    if (!quality)
        throw new Error(`Unsupported triad structure: ${key}`);
    return quality;
}
export function identifySeventh(notes) {
    if (notes.length !== 4)
        throw new Error("Seventh chord must contain four notes");
    const rootPc = pitchClass(notes[0]);
    ...
    const quality = map[key];
    if (!quality)
        throw new Error(`Unsupported seventh structure: ${key}`);
    return quality;
}
